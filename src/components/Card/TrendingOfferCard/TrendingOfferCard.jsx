...
import Styles from "./TrendingOfferCard.module.scss";
import { BlackGuy, Dress } from "~/assets";
import { ExploreButton } from "~/components/Buttons";

function Card({ discount, image, className }) {  // Removed isActive and onHover since we handle via className now
    return (
        <div className={clsx(Styles.card, className)}>
            <div className={Styles.innerContent}>
                <div className={Styles.content}>
                    ...
                    <h2 className={clsx(Styles.title)}>Min {discount}% off</h2>
...
function TrendingOfferCard() {
    ...
    const data = [
        { discount: 60, image: BlackGuy },
        { discount: 50, image: Dress },
        { discount: 80, image: Dress },
        { discount: 70, image: BlackGuy },
    ];
    const handleNext = () => {
    ...
    const effectiveData = [data[prevIndex], data[active], data[nextIndex]];

    return (
        <div className={clsx(Styles.trendingOfferCard)}>
            <button
                onClick={handlePrev}
            ...
            >
                ...
                >
                    <path
                        ...
                        d="M14 18L15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6z"
                    />
                </svg>
            </button>
            {effectiveData.map((x, index) => (
                <Card
                    key={index}  // Temporary key; in production, use unique IDs if available
                    discount={x.discount}
                    image={x.image}
                    className={clsx(
                        index === 0 && Styles.prev,
...
export default React.memo(TrendingOfferCard);